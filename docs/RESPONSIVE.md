...
| 5 | **C7** — les cibles tactiles, et la colonne des pages légales | Bloc `@media (pointer: coarse)` en fin de `styles.css` : un iPad paysage rend l'en-tête desktop mais se touche au doigt, un seuil de largeur l'aurait manqué. Pastilles réseaux 34 → 44 px, burger 42 → 44, liens de nav, téléphone et `link-arrow` à 44 de haut, puces du hero à 44 de pas, pied de page : la marge des listes passe dans la hauteur du lien ; fil d'Ariane et coordonnées du contact : zone invisible en `::after`, mise en page intacte. Liens pris dans une phrase laissés à la taille du texte (exemptés, la sonde les compte encore sur les pages légales). Pages légales : fiche 51 → 42 rem, corps 15,2 → 17 px, texte plafonné à 33 rem — **115–120 → 82–86 caractères par ligne** mesurés lettre par lettre. `verifier-desktop.py` : l'attente de `decode()` est bornée (elle pendait sur les photos `lazy` masquées des chambres) ; **24/24 rendus identiques**. | ok | **0** sur les 4 pages (390 et iPad 1024) | 26–76 | 0–13 | non remesuré |
